...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'Strands Sounds API' });
});
...
app.get('/song/:id', async (req, res) => {
  ...
  try {
    ...
    const description = `🎵 ${song.style} • Create your own AI music free on Strands Sounds`;
...
<head>
  ...
  <title>${title} | Strands Sounds</title>
  ...
  <meta property="og:site_name" content="Strands Sounds">
...
<body>
  <p>Redirecting to <a href="${config.frontendUrl}?song=${song.id}">Strands Sounds</a>...</p>
</body>
</html>`);
  } catch (error) {
...
app.listen(config.port, '0.0.0.0', () => {
  console.log(`Strands Sounds Server running on http://localhost:${config.port}`);
